...
if(isset($_GET['kategori'])){
    ...
    for($i=0; $i<count($ktg); $i++){
        $kateg[$i] = "SELECT * FROM produk WHERE kategori = '".$ktg[$i]."'";
    }
    $res_kategori = $kateg[0];
    if(count($kateg)>1){
        for($i=1;$i<count($kateg);$i++){
            $res_kategori = $res_kategori." UNION ".$kateg[$i];
        }
    }
    // deboogggg
    // echo ($res_kategori);
} else {
    $res_kategori = "SELECT * FROM produk";
}

if ($_GET['harga-min']==NULL && $_GET['harga-maks']==NULL){
    $res_harga = "SELECT * FROM produk";
}
else if ($_GET['harga-min']!=NULL && $_GET['harga-maks']==NULL){
    $min = $_GET['harga-min'];
    $res_harga = "SELECT * FROM produk where harga_baru>=".$min;
}
else if ($_GET['harga-min']==NULL && $_GET['harga-maks']!=NULL){
    $maks = $_GET['harga-maks'];
    $res_harga = "SELECT * FROM produk where harga_baru<=".$maks;
} else {
    $min = $_GET['harga-min'];
    $maks = $_GET['harga-maks'];
    $res_harga = "SELECT * FROM produk where harga_baru>=".$min. " AND "."harga_baru<=".$maks;
}
// Pengelompokan urutan
// KURANG POPULER, DISKON
if($_GET['urutan']=='baru'){
    $res_urutan = "ORDER BY id_produk DESC";
} else if ($_GET['urutan']=='rendah'){
    $res_urutan = "ORDER BY harga_baru ASC";
} else if ($_GET['urutan']=='tinggi'){
    $res_urutan = "ORDER BY harga_baru DESC";
} else if ($_GET['urutan']=='diskon'){
    $res_urutan = "ORDER BY diskon DESC";
}
// Pengelompokan Size kurang
$resultproduk = mysqli_query($conn, $res_kategori." INTERSECT ".$res_harga." ".$res_urutan);
$row = 1;
